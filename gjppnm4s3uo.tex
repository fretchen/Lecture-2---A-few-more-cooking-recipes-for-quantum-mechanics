...
We now  would like to understand than if we can disentangle the information about the different labels. In general we will say that the quantum system is formed by two subsystems $S_1$ and $S_2$. If they are independent we can write each of them as:
...
Separable states thus only describes a small subset of all possible states. All other states are called \textit{entangled}. As for the shared coin