...
We now  would like to understand than if we can disentangle the information about the different labels. Naively, we can now  associate with Alice one set of outcomes and describe it by some state $\ket{psi_{A}}$ and the Bob has another set $\ket{psi_{A}}$:
\begin{align}
				\ket{\psi_A}&=\sum_m^M \alpha_\uparrow \ket{\alpha_m},\\
				\ket{\psi_B}&=\sum_n^N b_n \ket{\beta_n}.
